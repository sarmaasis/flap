...
export type SettingsAppProps = {
  /** When set (route wrappers), lock the surface instead of inferring from the path. */
  forcedSurface?: SettingsSurface;
};
...
export default function SettingsApp({ forcedSurface }: SettingsAppProps) {
  const clerk = useClerk();
  const { isLoaded: authLoaded, isSignedIn, getToken } = useAuth();
  const [surface, setSurface] = useState<SettingsSurface>(() => forcedSurface ?? detectSurface());
  const [tab, setTab] = useState<Tab>(() => initialTab(forcedSurface ?? detectSurface()));
  ...
  const [keyCounts, setKeyCounts] = useState<{ live: number; test: number; total: number }>({ live: 0, test: 0, total: 0 });
    ...
    try {
      switch (next) {
        ...
        case "developers": {
          ...
          setKeys(k.keys);
          setKeyCounts(k.counts ?? {
            live: k.keys.filter((x) => x.mode !== "test").length,
            test: k.keys.filter((x) => x.mode === "test").length,
            total: k.keys.length,
          });
          ...
          <div className={cn("w-full max-w-none gap-8", tw.stack)}>
            ...
            <section>
              ...
              <form className={tw.rowForm} onSubmit={(e) => {
                ...
                void api.createKey(input.value || "Transactional", keyMode).then((res) => {
                  setNewToken(res.key.token ?? "");
                  input.value = "";
                  return refresh();
                });
              }}>
              ...
              {keys.length ? <div className="overflow-x-auto"><table className={settingsTable}><thead><tr><th>Name</th><th>Mode</th><th>Prefix</th><th /></tr></thead><tbody>
                {keys.map((k) => (
                  <tr key={k.id}>
                    <td>{k.name}</td>
                    <td><Badge variant={k.mode === "test" ? "secondary" : "default"}>{k.mode === "test" ? "Test" : "Live"}</Badge></td>
                      ...
                      <td className={rowActions}>
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            void api.redeliverWebhook(w.id)
                              .then(async () => {
                                setNotice("Redelivery attempted.");
                                const res = await api.webhookDeliveries(w.id);
                                setWebhookDeliveries((prev) => ({ ...prev, [w.id]: res.deliveries }));
                                setWebhookExpanded(w.id);
                              })
                              .catch((ex) => setErr(ex instanceof Error ? ex.message : "Redelivery failed."))
                              .finally(() => setRedeliverBusy(null));
                          }}
                        >
                                    ...
                                    <td>
                                      <button
                                        ...
                                        onClick={() => {
                                          ...
                                          void api.redeliverWebhook(w.id, d.id)
                                            .then(async () => {
                                              setNotice("Delivery re-posted.");
                                              const res = await api.webhookDeliveries(w.id);
                                              setWebhookDeliveries((prev) => ({ ...prev, [w.id]: res.deliveries }));
                                            })
                                            .catch((ex) => setErr(ex instanceof Error ? ex.message : "Redelivery failed."))
            ...
            <section className={tw.settingsCard}>
              ...
              <form className={stackForm} onSubmit={(e) => {
                ...
                <Checkbox
                  ...
                  label="Enable browser notifications for new mail"
                />
                <label>
                    ...
                    >
                      {isCurrent
                        ...
                        : checkoutBusy === "scale"
                          ...
                          : available
                            ? "Checkout Scale"
                            : "Scale checkout unavailable"}
                    </Button>
                    {!available ? (
                      <Button size="sm" variant="outline" type="button" onClick={() => go("/pricing")}>
